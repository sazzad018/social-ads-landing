...
const SuccessStories: React.FC = () => {
  return (
    <section id="case-studies" className="py-16 lg:py-24 bg-white border-t border-gray-100">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
           {SUCCESS_STORIES_DATA.stories.map((story, index) => (
             <div key={index} className="bg-white rounded-xl border border-gray-100 shadow-sm p-8 hover:shadow-lg transition-all duration-300 hover:-translate-y-1">
                <h3 className="text-xl font-bold text-gray-900 mb-6">{story.name}</h3>

                <div className="mb-5">
                   <h4 className="text-orange-600 font-bold mb-2 text-[15px]">সমস্যা ছিল</h4>
                   <p className="text-gray-600 text-sm leading-relaxed">{story.problem}</p>
                </div>

                <div className="mb-8">
                   <h4 className="text-orange-600 font-bold mb-2 text-[15px]">সমাধান করা হয়েছে</h4>
                ...
                <div className="border-t border-gray-100 pt-6">
                   ...
                   <div className="grid grid-cols-3 gap-2 text-center">
                      {story.results.map((result, idx) => (
                        <div key={idx}>
                           <div className="text-amber-500 font-bold text-xl lg:text-2xl mb-1">{result.value}</div>
